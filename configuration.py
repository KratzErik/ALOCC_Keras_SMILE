...
class Configuration(object):
    ...
    if dataset == "dreyeve":
        ...
        ae_n_conv_layers_per_module = 2 # number of conv. layers in each module (between each pool layer/dim reduction)
        ae_n_dense_layers = 1 # number of dense layers in
        ae_z_dim = 256
        ae_n_dense_units = ae_z_dim
        ae_filter_size = 4
        ...
        d_pool_size = 2
        d_channels_first_layer = 16
        d_channels = [16 * 2**i for i in range(d_n_conv_modules)] # TODO: replace number with d_channels_first_layer
        d_use_batch_norm = True
        d_use_dropout = False
        d_dropout_rate = 0.1
